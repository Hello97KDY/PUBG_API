...
# username = 'breakthebalance'#'zpdldptmxlsw'
...
# pubg_database = pymysql.connect(host=host, port=3306, user='root', passwd=password, db=db, charset='utf8')
# cursor = pubg_database.cursor()

# sql = f"SELECT DISTINCT id, user_name FROM pubg_db_test where user_name = '{username}'"
# player_match_ids_df = pd.read_sql(sql,con = pubg_database)
...
def get_positions(username, url_list):
    positions = []
...
def draw_map(positions, plt_map_name, map_image_path):
    scale_factor = 0.01
...
def main():
    ...
    positions, plt_map_name, map_image_path = get_positions(username, match_telemetries)

    draw_map(positions, plt_map_name, map_image_path)
